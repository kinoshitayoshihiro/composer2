...
class DurationDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        pc = torch.tensor(g['pitch_class'].tolist() + [0] * pad, dtype=torch.long)
        pos = torch.tensor(g['position'].tolist() + [0] * pad, dtype=torch.long)
        mask = torch.zeros(self.max_len, dtype=torch.bool)
        mask[:L] = 1
        return {'duration': dur, 'velocity': vel, 'pitch_class': pc, 'position_in_bar': pos}, dur, mask
...
class DurationDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(self.train_ds, batch_size=self.batch_size, shuffle=True, collate_fn=collate_fn)
