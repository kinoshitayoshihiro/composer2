...
try:
    ...
    from utilities.core_music_utils import sanitize_chord_label # ★ これを確実に使用する
...
DEFAULT_CONFIG = {
    ...
    "default_part_parameters": {
        "piano": {
            "instrument": "Piano", "default_humanize": False,
        ...
        "guitar": {
            ...
            "default_num_strings": 6, "default_target_octave": 3, "default_velocity": 70,
        },
        "bass": {
            "instrument": "AcousticBass", "default_humanize": False,
            "default_humanize_style_template": "default_subtle",
            "default_rhythm_key": "bass_quarter_notes", "default_velocity": 80,
            "default_octave": 2, "default_weak_beat_style": "root",
        ...
        "chords": {"instrument": "ElectricPiano", "default_humanize": False, "default_velocity": 60},
    },
    "rng_seed": None,
}
...
def translate_and_merge_params_from_emotion_data(
    processed_event_data: Dict[str, Any], # これは emotion_humanizer からの event 単位のデータ
    ...
    if arrangement_override_for_part: # オーバーライドでさらに上書き
        ...
        if ("options" in override_dict and "options" in final_params and
            isinstance(final_params["options"], dict) and isinstance(override_dict["options"], dict)):
    ...
    emotion_params_in_event = processed_event_data.get("emotion_params", {}) # emotion_humanizer.py からの出力名
...
def prepare_stream_for_generators(
    ...
    for sec_name, sec_data in sorted_sections:
        ...
        for event_idx, event_from_humanizer in enumerate(
            ...
            chord_for_voicing_from_yaml = event_from_humanizer.get("chord_symbol_for_voicing")
            bass_for_voicing_from_yaml = event_from_humanizer.get("specified_bass_for_voicing")

            # ▼▼▼ ここで sanitize_chord_label を適用 (最終防衛ライン) ▼▼▼
            final_sanitized_chord_symbol_for_generator: Optional[str] = "Rest" # デフォルトRest
            ...
            if temp_label_to_sanitize and str(temp_label_to_sanitize).strip().lower() != "rest":
                # ベース音が別指定されている場合、それを考慮してサニタイズ
                if bass_for_voicing_from_yaml and \
                   str(bass_for_voicing_from_yaml).strip().lower() not in ["", "rest", "r", "n.c.", "nc", "none"]:
                    # "C" と "Bb" -> "C/Bb" として sanitize_chord_label に渡す
                    combined_label = f"{temp_label_to_sanitize}/{bass_for_voicing_from_yaml}"
                    logger.debug(f"ModComp (prepare_stream): Sanitizing combined '{combined_label}' (orig_yaml: '{chord_for_voicing_from_yaml}' / '{bass_for_voicing_from_yaml}')")
                    sanitized_result = sanitize_chord_label(combined_label)
                else:
                    # ベース音指定なし、または無効なベース音の場合はコード部分のみサニタイズ
                    logger.debug(f"ModComp (prepare_stream): Sanitizing single '{temp_label_to_sanitize}' (orig_yaml: '{chord_for_voicing_from_yaml}', bass_yaml: '{bass_for_voicing_from_yaml}')")
                ...
                if sanitized_result and sanitized_result != "Rest":
                    ...
                    if '/' in sanitized_result:
                        ...
                        final_sanitized_bass_for_generator = parts[1] if len(parts) > 1 else None
                    else: # スラッシュなしで返ってきた場合 (例: "C7", "Am")
                        final_sanitized_chord_symbol_for_generator = sanitized_result
                        final_sanitized_bass_for_generator = None # ベースは吸収されたか、元々ない
            ...
            logger.debug(f"  ModComp: Final for gen: Chord='{final_sanitized_chord_symbol_for_generator}', Bass='{final_sanitized_bass_for_generator}' (From YAML: Chord='{chord_for_voicing_from_yaml}', Bass='{bass_for_voicing_from_yaml}')")
            # ▲▲▲ サニタイズ処理ここまで ▲▲▲
            ...
            blk_data = {
                ...
                "emotion_params": event_from_humanizer.get("emotion_profile_applied", {}), # emotion_humanizer.pyからの出力
                "part_params": {},
            }
            ...
            for part_name, generate_flag in main_config.get("parts_to_generate", {}).items():
                if generate_flag:
                    ...
                    part_override_model = get_part_override(arrangement_overrides, sec_name, part_name)
...
def run_composition(
    ...
    arrangement_overrides: Overrides = Overrides(root={}) # Pydanticモデルのインスタンス
    override_file_path_to_load: Optional[Path] = None

    # Overrideファイルのロード処理 (変更なし)
    if cli_args.overrides_file: override_file_path_to_load = cli_args.overrides_file
    elif Path("data/arrangement_overrides.json").exists(): override_file_path_to_load = Path("data/arrangement_overrides.json")
    elif Path("data/arrangement_overrides.yaml").exists(): override_file_path_to_load = Path("data/arrangement_overrides.yaml")
    elif Path("data/arrangement_overrides.yml").exists(): override_file_path_to_load = Path("data/arrangement_overrides.yml")
    ...
    ts_obj_score = get_time_signature_object(global_ts_str) # core_music_utils を使用
    final_score.insert(0, ts_obj_score if ts_obj_score else meter.TimeSignature("4/4")) # フォールバック
    final_score.insert(0, key.Key(global_key_tonic_val, global_key_mode_val.lower()))

    # processed_blocks の準備 (prepare_stream_for_generators を呼び出す)
    proc_blocks = prepare_stream_for_generators(
        processed_chordmap_data, main_cfg, rhythm_lib_data, arrangement_overrides
    )
    if not proc_blocks:
        logger.error("No blocks to process from processed_chordmap_data. Aborting.")
        return

    # ChordVoicer と各ジェネレータの初期化 (変更なし)
    ...
    for part_name, generate_flag in main_cfg.get("parts_to_generate", {}).items():
        ...
        except: instrument_obj = m21instrument.Piano() # フォールバック

        if part_name == "piano":
            gens[part_name] = PianoGenerator(rhythm_library=rhythm_lib_for_instrument, chord_voicer_instance=cv_inst,
                                             default_instrument_rh=instrument_obj, default_instrument_lh=instrument_obj,
                                             global_tempo=global_tempo_val, global_time_signature=global_ts_str)
        elif part_name == "drums":
            gens[part_name] = DrumGenerator(lib=rhythm_lib_for_instrument, tempo_bpm=global_tempo_val, time_sig=global_ts_str)
        elif part_name == "guitar":
            gens[part_name] = GuitarGenerator(rhythm_library=rhythm_lib_for_instrument, default_instrument=instrument_obj,
                                              global_tempo=global_tempo_val, global_time_signature=global_ts_str)
        elif part_name == "bass":
            gens[part_name] = BassGenerator(rhythm_library=rhythm_lib_for_instrument, default_instrument=instrument_obj,
                                            global_tempo=global_tempo_val, global_time_signature=global_ts_str,
                                            global_key_tonic=global_key_tonic_val, global_key_mode=global_key_mode_val,
                                            rng_seed=main_cfg.get("rng_seed"))
        elif part_name == "melody":
            gens[part_name] = MelodyGenerator(rhythm_library=rhythm_lib_for_instrument, default_instrument=instrument_obj,
                                              global_tempo=global_tempo_val, global_time_signature=global_ts_str,
                                              global_key_signature_tonic=global_key_tonic_val,
                                              global_key_signature_mode=global_key_mode_val)
        elif part_name == "vocal" and main_cfg["parts_to_generate"].get("vocal"):
            gens[part_name] = VocalGenerator(default_instrument=instrument_obj, global_tempo=global_tempo_val, global_time_signature=global_ts_str)
    ...
    for p_n, p_g_inst in gens.items():
        if p_g_inst and main_cfg["parts_to_generate"].get(p_n):
            ...
            try:
                ...
                if p_n == "vocal":
                    vocal_params_for_compose = (proc_blocks[0]["part_params"].get("vocal") if proc_blocks 
                                                else main_cfg["default_part_parameters"].get("vocal", {}))
                    ...
                    midivocal_p_str_call = cli_args.vocal_mididata_path or vocal_data_paths_call.get("midivocal_data_path")
                    ...
                    if midivocal_data_for_compose_list:
                        ...
                        logger.warning(f"Vocal generation skipped: No MIDI data from '{midivocal_p_str_call}'.")
                        continue
                else: # 他の楽器パート
                    if hasattr(p_g_inst, "compose"):
                        sig = inspect.signature(p_g_inst.compose)
                        compose_args = [proc_blocks] # proc_blocks を第一引数として渡す
                        compose_kwargs = {}
                        if "overrides" in sig.parameters: compose_kwargs["overrides"] = arrangement_overrides
                        ...
                        logger.error(f"Generator for {p_n} does not have a compose method.")
                        continue
                
                if isinstance(part_obj, stream.Score) and part_obj.parts:
                    for sub_part in part_obj.parts: # Scoreの場合、各Partを取り出す
                ...
                logger.error(f"Error in {p_n} generation: {e_gen}", exc_info=True)
    ...
    except exceptions21.Music21Exception as e_m21w:
        logger.error(f"Music21 MIDI write error to {out_fpath}: {e_m21w}", exc_info=True)
...
def main_cli():
    ...
    if args.settings_file and args.settings_file.exists():
        ...
        if custom_settings_data and isinstance(custom_settings_data, dict):
            def _deep_update(target_dict, source_dict): # 設定を再帰的にマージ
    ...
    if args.vocal_mididata_path: # Vocal MIDIデータパスのCLIオーバーライド
        if "vocal" not in effective_cfg["default_part_parameters"]: effective_cfg["default_part_parameters"]["vocal"] = {}
        if "data_paths" not in effective_cfg["default_part_parameters"]["vocal"]: effective_cfg["default_part_parameters"]["vocal"]["data_paths"] = {}
        effective_cfg["default_part_parameters"]["vocal"]["data_paths"]["midivocal_data_path"] = str(args.vocal_mididata_path)

    if args.rng_seed is not None: # RNGシードのCLIオーバーライド
        effective_cfg["rng_seed"] = args.rng_seed
        random.seed(args.rng_seed)
        logger.info(f"RNG Seed set to: {args.rng_seed}")

    # processed_chordmap_with_emotion.yaml のロード (変更なし)
    processed_chordmap_data_loaded = load_yaml_file(args.processed_chordmap_file, "Processed Chordmap with Emotion")
    if not processed_chordmap_data_loaded: sys.exit(1)
    ...
    try:
        ...
        if not rhythm_library_model:
            logger.critical(f"Failed to load rhythm library from {args.rhythm_library_file} (loader returned None). Exit.")
            sys.exit(1)
        rhythm_library_data_loaded = rhythm_library_model.model_dump() # Pydanticモデルから辞書へ
        logger.info(f"Rhythm library loaded and parsed from {args.rhythm_library_file}.")
    except FileNotFoundError:
        logger.critical(f"Rhythm library file not found: {args.rhythm_library_file}. Exit.")
        sys.exit(1)
    except ValueError as e_val: # Pydanticのバリデーションエラーなども含む可能性
        logger.critical(f"Error processing rhythm library {args.rhythm_library_file}: {e_val}. Exit.", exc_info=False) # exc_info=False でValueErrortracebackは抑制
        sys.exit(1)
    except Exception as e_rhythm_load:
        logger.critical(f"Unexpected error loading rhythm library {args.rhythm_library_file}: {e_rhythm_load}. Exit.", exc_info=True)
        sys.exit(1)
    if not rhythm_library_data_loaded or not isinstance(rhythm_library_data_loaded, dict):
        logger.critical("Rhythm library data invalid after loading. Exit.")
        sys.exit(1)

    # グローバル設定の最終決定 (chordmap優先、次にCLI、最後にデフォルト) (変更なし)
    ...
    if args.tempo is not None: # CLIでのテンポ指定があれば最優先
    ...
    logger.debug(f"Is 'sanitize_chord_label' in globals()? {'sanitize_chord_label' in globals()}")
    if 'sanitize_chord_label' in globals(): logger.debug(f"Type of 'sanitize_chord_label': {type(globals()['sanitize_chord_label'])}")
    else: logger.error("'sanitize_chord_label' NOT FOUND in globals() before run_composition!")

    try:
        run_composition(args, effective_cfg, processed_chordmap_data_loaded, rhythm_library_data_loaded)
    except SystemExit: # sys.exit() が呼ばれた場合
        raise
